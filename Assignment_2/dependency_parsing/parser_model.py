...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight = nn.Parameter(
            nn.init.xavier_uniform_(torch.zeros(n_features*self.embed_size, hidden_size)))
        ...
        self.hidden_to_logits_weight = nn.Parameter(
            nn.init.xavier_uniform_(torch.zeros(hidden_size, n_classes)))
    ...
    def embedding_lookup(self, w):
        ...
        x = w.flatten()
        x = self.embeddings[x].view(w.shape[0], w.shape[1], -1)
    ...
    def forward(self, w):
        ...
        x = self.embedding_lookup(w).flatten(1, 2)
        h = self.relu(x.matmul(self.embed_to_hidden_weight) +
                      self.embed_to_hidden_bias)
        h = self.dropout(h)
        l = h.matmul(self.hidden_to_logits_weight)+self.hidden_to_logits_bias
        # logits = self.softmax(l)
        logits = l
